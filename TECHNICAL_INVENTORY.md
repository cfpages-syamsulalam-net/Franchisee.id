...
- `runDryRun()` / `enqueueJobs()` / `runJobs()` / `retryBatch()` / `retryJob()` / `retryFailedJobs()`: Calls the admin-only OCR actions with explicit copy that dry-run is a real one-asset OCR run. `runJobs()` now creates a persisted server-side scheduler batch when an active scheduler exists so OCR continues while the tab/browser is inactive; if a failed/paused scheduler batch still has unfinished jobs, it resumes that batch instead of creating another one. New/retried scheduler batches immediately run one dashboard chunk so the click produces visible progress before the delayed scheduler trigger arrives. Only when no scheduler is active does it fall back to the browser continuous loop with a clear keep-tab-open warning. Batch retry skips already-succeeded jobs, returns failed/stale-running jobs in that batch to pending, and then reschedules after scheduler credential/URL fixes; per-row failed-job OCR retry runs that job immediately; batch failed retry requeues up to 100 failed jobs. Job execution/retry buttons are disabled when no provider is currently runnable, including when the only active provider is still in an active cooldown window; expired cooldown providers are treated as runnable again.
...
- `window.FranchiseDashboardOcrBatches.createRenderer(deps)`: Creates pure render helpers for persisted OCR batch rows, progress bars, Retry/Refresh buttons, and dynamic structured scheduler countdown/status chips such as `Menunggu 35s` and `Memproses`.
...
- Provides the static legal-page layout, Franchisee.id header/footer, favicon metadata, absolute canonical URL metadata, readable responsive CSS, and reciprocal navigation between homepage, Privacy Policy, Terms of Service, and contact page.
- Used by Google OAuth verification-facing legal pages so the Privacy/Terms presentation stays consistent with the public site theme.
...
*Admin-only OCR dashboard job action module.*
- `handleRetryOcrJob(db, auth, data, env, options)`: Resets one failed/needs-review/no-text OCR job to `pending`, then immediately runs the scoped job through `_ocr-job-runner.js` so row-level retry is an actual OCR retry, not only a requeue.
- `handleMarkOcrJobNoText(db, auth, data)`: Manual image-review resolution that marks a failed/needs-review OCR job as final `no_text` and writes an audit event.
- `handleRetryFailedOcrJobs(db, auth, data)`: Resets up to 100 failed jobs back to `pending` for bounded retry without deleting attempt history.
