...
- `refreshBatchProgress(db, batchId, options)`: Recomputes assigned/processed/succeeded/failed/needs-review counts from `ocr_jobs`, updates `ocr_batch_runs`, supports `paused_rate_limit`/`paused_quota` while work remains pending, and returns the masked batch row.
