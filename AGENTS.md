...
- Make the suggestion pass dependency-aware, not feature-isolated. For every implemented decision, inspect at least one step upstream and downstream: what creates the data/URL/state, what consumes or links to it, what old shape remains, how existing users or crawlers recover, and what operational or test coverage proves the transition. A logically necessary, low-risk follow-through such as redirects, internal-link migration, stored/generated artifact cleanup, navigation reachability, or regression coverage is part of completing the current feature and must be implemented in the same session rather than omitted or left as an optional suggestion.
...
- Treat canonical URL and route changes as migrations, not isolated link edits. Before marking one complete: inventory every producer and consumer; update templates, renderers, browser scripts, copied legacy content, tracked/generated snapshots, navigation, breadcrumbs, schema, sitemap/canonical metadata where relevant; add permanent redirects for every known old path/query/alias shape while preserving unrelated query state; centralize alias normalization; run a repo-wide old-URL scan; and add a focused redirect/link regression check. If static redirect syntax cannot match the old URL shape, implement the narrowest server/edge handler needed instead of silently relying on client JavaScript.
