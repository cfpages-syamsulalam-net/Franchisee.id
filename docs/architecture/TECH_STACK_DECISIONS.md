...
pnpm run build:d1:franchises:bridge
...
- Does not write root `/peluang-usaha/*.html` bridge files or `json/d1-generated-pages-manifest.json` by default. Astro now owns the deployable public pages in `dist`, so default builds avoid mass-generated source diffs.
- `pnpm run build:d1:franchises:dry` still renders the bridge pages in memory for validation without writing files.
- `pnpm run build:d1:franchises:bridge` explicitly writes `/peluang-usaha/index.html`, flat `/peluang-usaha/{slug}.html` bridge files, and `json/d1-generated-pages-manifest.json` when the legacy bridge output itself needs review/regeneration.
- Bridge pruning only runs in explicit bridge-write mode and only removes pages that were previously generated by this D1 builder and still contain the `d1-generated:franchisee.id` marker.
