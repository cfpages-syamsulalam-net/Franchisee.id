...
- Older document suggestions that lack complete embedded evidence are enriched at dashboard read time from `franchise_asset_knowledge` by `_dashboard-review-evidence.js`, so each field can still show sanitized excerpt context and a brochure image hover preview when the source asset URL exists.
- Evidence excerpts include a highlighted `basis` snippet when `_proposal-evidence.js` can match the field-specific phrase. Weak numeric matches, such as a standalone `8` in unrelated financial projection text, intentionally show a warning instead of pretending to prove `Jumlah staff minimum`.
...
- Existing canonical listing fields: `fee_license_idr` (`Biaya Kemitraan Awal`, including franchise fee, license fee, biaya lisensi, biaya franchise, biaya kemitraan, biaya join, joining fee, or investasi kemitraan wording), `fee_capex_idr`, `fee_construction_idr`, `total_investment_idr`, `min_investment_idr`, `max_investment_idr`, `contract_duration_months`.
- Ambiguous package terms such as `paket kemitraan` should not automatically populate `fee_license_idr`; they may include equipment, stock, renovation, or other bundle components and need clearer wording or manual review.
...
- `rent_cost_text`
- `contract_duration_months`
- `fee_license_idr` for Biaya Kemitraan Awal wording: franchise fee, license fee, biaya lisensi, biaya franchise, biaya kemitraan, biaya join, joining fee, or investasi kemitraan.
- `fee_capex_idr`
