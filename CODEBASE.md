...
6. Account name, email, and password are edited one field at a time. Name/email saves update Clerk first, then D1 `users` plus profile identity fields, and password changes use ClerkJS so Google-only users can add password login while password users can change it.
