...
| `js/dashboard-admin.js` | Client controller for `/dashboard`. Boots Clerk auth through `window.FranchiseAuth`, fetches `/dashboard-data`, renders overview/outreach/quality/claim/edit/lead/health sections, handles manual WhatsApp outreach logging, edit suggestions, admin review actions, and masked auth-debug copy/refresh controls. | `src/pages/dashboard/index.astro`, `functions/dashboard-data.js`, `js/auth-clerk.js` |
| `src/pages/dashboard/index.astro` | Static protected dashboard shell at `/dashboard`. Shows a same-URL login-only Clerk form for unauthenticated staff/admin users, keeps operational panels locked until `/dashboard-data` authorizes D1 `staff` or elevated `admin`, renders dashboard markup, dashboard-only styles, and the masked auth debug panel shell. Runtime data rendering and actions live in `js/dashboard-admin.js`. Its styles are self-contained plus existing Elementor/Auth CSS; it does not depend on the missing legacy Astra dynamic CSS file. | `js/dashboard-admin.js`, `functions/dashboard-data.js`, `js/auth-clerk-debug.js`, `js/auth-clerk.js`, `css/auth-clerk.css`, Clerk, D1 |
...
3. `js/dashboard-admin.js` calls `/dashboard-data` with a Clerk bearer token when a session exists.
