...
| `scripts/copy-legacy-static.mjs` | Post-Astro build copier for the hybrid deployment. Copies legacy static HTML/assets/directories into `dist` without overwriting existing Astro output and skips legacy `/peluang-usaha`, legacy `/category`, and source `public/` so Astro owns generated routes, `/category` stays redirect-only, and root public assets are copied by Astro. | `package.json` `copy:legacy-static`, Cloudflare Pages output `dist`, legacy root static export |
...
| `src/pages/rekomendasi/index.astro` | D1-backed static archive for `/rekomendasi`, ordered by verification/status and listing completeness. | `src/lib/franchise-static.ts`, `json/d1-franchise-static-data.json` |
| `src/pages/populer/index.astro` | D1-backed static archive for `/populer`, ordered by a deterministic popularity proxy until real view/lead/payment metrics exist. | `src/lib/franchise-static.ts`, `json/d1-franchise-static-data.json` |
| `src/pages/abjad/index.astro` | D1-backed static archive for `/abjad`, ordered alphabetically by brand name. | `src/lib/franchise-static.ts`, `json/d1-franchise-static-data.json` |
| `src/pages/kategori/index.astro` | D1-backed static category index for `/kategori`, showing one card per category with listing counts. | `src/lib/franchise-static.ts`, `json/d1-franchise-static-data.json` |
| `src/pages/kategori/[slug].astro` | D1-backed category archive route for `/kategori/{slug}.html`, including aliases such as `makanan-minuman-fb`, `perhotelan-travel`, `properti-furniture`, `teknologi-digital`, and `jasa-layanan`. | `src/lib/franchise-static.ts`, `json/d1-franchise-static-data.json` |
| `src/pages/[categorySlug].astro` | Compatibility top-level category archive route for legacy category slugs such as `/makanan-minuman-fb`. | `src/lib/franchise-static.ts`, `json/d1-franchise-static-data.json` |
| `public/_redirects` | Cloudflare Pages redirect rules. Redirects legacy `/category/*` paths to canonical Indonesian `/kategori/:splat` URLs. | Astro public asset copy, Cloudflare Pages |
...
3. `src/pages/peluang-usaha/index.astro` renders `/peluang-usaha/index.html` and uses a CSS-only fallback when cover/logo images are missing.
...
5. `src/pages/rekomendasi/index.astro`, `src/pages/populer/index.astro`, and `src/pages/abjad/index.astro` reuse the same listing template with route-specific sorting and metadata.
6. `src/pages/kategori/index.astro`, `src/pages/kategori/[slug].astro`, and `src/pages/[categorySlug].astro` generate category index/archive pages from the D1 snapshot plus legacy slug aliases. `/category/*` redirects to `/kategori/*` and must not be generated as duplicate canonical pages.
7. `scripts/copy-legacy-static.mjs` copies the legacy static export into `dist` without overwriting Astro-generated files, so old CSS/JS/images/static pages remain available.
8. The copy step skips legacy root `/peluang-usaha` to avoid route conflicts with Astro's D1-backed `/peluang-usaha` output and skips legacy `/category` so old category URLs are handled only by the `/category/*` to `/kategori/*` redirect.
