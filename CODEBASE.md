...
| `css/dashboard.css` | Dashboard-only layout and component styling. Owns metric cards, icon tab controls, panel grids, dashboard tables/forms/buttons/badges, debug panel, and responsive dashboard behavior. | `src/pages/dashboard/index.astro`, `js/dashboard-admin.js` |
...
3. `js/dashboard-admin.js` calls `/dashboard-data` with a Clerk bearer token when a session exists and presents details in icon-led tabs: Outreach, Data Quality, Review, and Operations.
...
5. `_dashboard-queries.js` reads overview metrics, unclaimed outreach rows, pending claims, pending edit suggestions, editable listing options, data-quality warnings, lead counts, system-health probes, and publish queue state from D1.
