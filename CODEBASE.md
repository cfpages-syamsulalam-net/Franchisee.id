...
| `functions/profile-data.js` | Protected profile center API for `/profil`. GET returns the current D1 user, roles, franchisee/franchisor profiles, owned listings, claim history, franchisee recommendations, D1 saved opportunities, franchisee inquiry history, franchisor lead inbox, and completion state. POST validates account/profile/listing edits, public role additions, saved-opportunity save/remove, franchisee inquiry creation, and franchisor lead status updates with Zod; updates Clerk identity then D1 for account fields; writes D1 profile edits; lets public users add the missing `franchisee`/`franchisor` role; returns action CTA metadata when a recoverable missing-profile state blocks save/inquiry actions; creates `franchise_leads` from profile recommendations; rate-limits owner listing edits to one per listing per 6 hours; writes audit events; and queues static rebuild requests for public listing changes. | `src/pages/profil/index.astro`, `js/profile-page.js`, `functions/_clerk-auth.js`, `functions/_site-publish-queue.js`, D1 `users`, `user_roles`, `franchisee_profiles`, `franchisor_profiles`, `franchises`, `franchise_claims`, `franchise_leads`, `franchise_saved_opportunities`, `audit_events`, `site_rebuild_requests` |
...
| `functions/dashboard-data.js` | Thin protected Franchisee.id dashboard API router. Requires D1 role `staff` or elevated `admin`, dispatches validated dashboard actions, returns shared editable-field definitions to the client, and composes the GET response from dashboard query modules. | `functions/_dashboard-schemas.js`, `functions/_dashboard-queries.js`, `functions/_dashboard-actions.js`, `functions/_dashboard-utils.js`, `src/pages/dashboard/index.astro`, `js/dashboard-admin.js` |
...
| `functions/_dashboard-schemas.js` | Dashboard action schemas, site constants, shared editable-listing field definitions, shared field normalization, and update statement builder for dashboard listing edits. | `functions/dashboard-data.js`, `functions/_dashboard-actions.js`, `functions/_shared-schemas.js`, Zod |
...
| `functions/_dashboard-queries.js` | Dashboard read model for overview metrics, persisted data-quality warnings with live fallback, publish state, unclaimed outreach queue plus outreach summary counts, pending claims, full editable listing snapshots, recent outreach, edit suggestions, lead summary, and system health. | `functions/dashboard-data.js`, `functions/_dashboard-utils.js`, `functions/_quality-checks.js`, D1 |
| `functions/_dashboard-actions.js` | Dashboard write workflows for manually confirmed WhatsApp outreach logging, guided listing edit suggestions, data-quality refresh, admin edit suggestion review, and admin claim review. Writes audit events and queues static rebuilds for approved public-page-affecting changes. | `functions/dashboard-data.js`, `functions/_dashboard-schemas.js`, `functions/_dashboard-queries.js`, `functions/_dashboard-utils.js`, `functions/_quality-checks.js`, `functions/_site-publish-queue.js`, D1 |
...
| `js/profile-page.js` | Client controller for `/profil`. Requires Clerk session, fetches `/profile-data`, renders side-tab profile sections for summary/account plus role-allowed franchisee/opportunities/franchisor/listings/leads/claims areas, offers missing public role add-on CTAs with confirmation, renders franchisee recommendations/D1 saved opportunities/inquiry history, syncs old local saved opportunities after login, creates one-click franchise inquiries with Clerk bearer tokens, edits name/email one field at a time through the protected profile API, lets Google-only accounts add an email/password login through Clerk, lets password users change password through Clerk, uploads owner listing media through `/profile-upload`, updates franchisor lead status, submits profile mutations, and exposes admin/staff dashboard shortcut only when D1 roles allow it. | `src/pages/profil/index.astro`, `functions/profile-data.js`, `functions/profile-upload.js`, `js/auth-clerk.js`, `css/profile.css`, browser `localStorage`, ClerkJS user password APIs |
| `js/opportunity-save.js` | Public save-opportunity controller for generated franchise directory cards and detail pages. On click, ensures a Clerk session, redirects anonymous users to `/login?next=...`, posts D1 saved-opportunity save/remove actions to `/profile-data`, updates icon-only/card and detail button states, and renders CTA-backed feedback when the API returns next-action metadata. | `css/opportunity-save.css`, `src/lib/franchise-static.ts`, `functions/profile-data.js`, public franchise templates |
...
5. Franchisor users can use the `Leads` tab to review incoming `franchise_leads` for owned listings, open email/WhatsApp contact shortcuts, and update lead status.
6. Owner media uploads from the listing panel write to R2 through `/profile-upload`, create `franchise_assets`, update the listing media URL field, and queue static rebuild requests.
7. `/profile-data` maps Clerk to D1 through `functions/_clerk-auth.js`, returns D1-authoritative roles and ownership rows, and exposes `/dashboard/` only when the user has `admin` or `staff`.
8. Account name, email, and password are edited one field at a time. Name/email saves update Clerk first, then D1 `users` plus profile identity fields, and password changes use ClerkJS so Google-only users can add password login while password users can change it.
9. Franchisee/franchisor profile edits update existing D1 profile rows and write audit events; first-time completion still belongs to `/daftar/`.
10. Owner listing edits update whitelisted public fields directly in D1, are limited to one edit per listing per 6 hours, write audit events, and enqueue `site_rebuild_requests` instead of triggering an immediate build.
...
5. `_dashboard-queries.js` reads overview metrics, unclaimed outreach rows, outreach summary counts, pending claims, pending edit suggestions, editable listing snapshots, persisted data-quality warnings with computed fallback, lead counts, system-health probes, and publish queue state from D1.
