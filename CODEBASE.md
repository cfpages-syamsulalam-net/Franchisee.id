...
| `js/auth-clerk.js` | Custom ClerkJS client integration. Replaces the legacy `/login` WPForms block with public login/register/verification panels, supports Google sign-in/sign-up through the hidden `/sso-callback/` route, completes Clerk OAuth redirect callbacks before token checks, stores selected public registration role and post-login destination across OAuth redirects before `/auth-sync`, supports a login-only staff/admin variant for `/dashboard`, keeps inactive auth panels hidden, powers `/register`, exposes `window.FranchiseAuth`, supplies bearer tokens to protected form writes, has a public publishable-key fallback if `/auth-config` is stale or empty, sets Clerk's script-load publishable-key global/attributes, loads locally copied ClerkJS before CDN fallbacks, and records sanitized auth debug snapshots/events for dashboard SSO diagnostics. | `/auth-config`, `/auth-sync`, `/login`, `/register`, `/dashboard`, `/sso-callback`, `/daftar`, `js/form-06-submit-validation.js`, `/clerk/clerk.browser.js` |
| `css/auth-clerk.css` | Custom auth UI styling built on existing Astra/global colors. Explicitly preserves `[hidden]` tab/panel behavior against legacy display rules and styles login/register switch prompts plus Google OAuth buttons with an inline Google logo mark. | `/login`, `/register`, `/dashboard`, `/daftar` |
...
| `src/pages/sso-callback/index.astro` | Hidden static Clerk OAuth callback route. Loads `js/auth-clerk.js`, lets `Auth.init()` run `clerk.handleRedirectCallback()`, and falls back to the saved pending destination if Clerk returns without navigating. This page should be listed in Clerk allowed redirect/application URLs but is not a separate login surface. | `js/auth-clerk.js`, Clerk Google OAuth, `/login`, `/register`, `/dashboard` |
...
6. The custom browser bootstrap handles Clerk OAuth callback parameters before token checks, then clears stale callback parameters or navigates to the saved post-login URL.
