...
| `js/form-07-init.js` | DOMContentLoaded bootstrap for form modules, HAKI conditional display, state restoration, role deep-link tab selection, Clerk login enforcement for `/daftar`, Clerk/D1 identity prefill for empty email/name fields, and compatibility globals. | All form modules, `/daftar/index.html`, `js/auth-clerk.js` |
...
| `daftar/index.html` | Static profile/listing completion page that loads `css/form-franchise.css`, all modular form scripts, and the custom Clerk/navbar runtime. Treat as integration context and preserve every field. Anonymous visitors are redirected to `/login?next=...` by `js/form-07-init.js`. |
...
9. `/daftar` requires Clerk login before completion, opens the requested role tab when `?role=` is present, and prefills empty email/name/PIC fields from Clerk/D1 identity.
10. Public navbar links are normalized by `js/auth-navbar.js`; logged-in users see name, D1 role badge, `/daftar` account link, and immediate red icon-only logout.
