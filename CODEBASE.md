...
| `js/auth-navbar.js` | Public navbar auth-state controller. On pages with legacy HFE nav menus, normalizes logged-out `Login`/`Daftar` links to `Masuk` and `Daftar Mitra` pointing at protected `/daftar/`, then replaces them after Clerk/D1 sync with Font Awesome account/logout icons, account name, D1 role badge, `/daftar` completion link, and red icon-only logout. | `js/auth-clerk.js`, `css/auth-clerk.css`, `/login`, `/daftar`, franchise directory/detail templates |
