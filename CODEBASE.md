...
| `src/pages/category/[slug].astro` | Compatibility category archive route for legacy `/category/{slug}` style links. | `src/lib/franchise-static.ts`, `json/d1-franchise-static-data.json` |
...
6. `src/pages/kategori/index.astro`, `src/pages/kategori/[slug].astro`, `src/pages/category/[slug].astro`, and `src/pages/[categorySlug].astro` generate category index/archive pages from the D1 snapshot plus legacy slug aliases.
...
8. The copy step skips legacy root `/peluang-usaha` to avoid route conflicts with Astro's D1-backed `/peluang-usaha` output.
