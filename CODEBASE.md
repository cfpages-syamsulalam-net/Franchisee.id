...
5. Email/password and Google OAuth are both supported. Public Google registration stores the selected `franchisee`/`franchisor` role across the OAuth redirect, then syncs it after the Clerk session is active.
6. After sign-up or sign-in, `/auth-sync` verifies the Clerk session, upserts D1 `users`, applies active `email_role_grants`, inserts self-assignable `franchisee`/`franchisor` roles when requested, and pushes the D1 role snapshot into Clerk metadata.
7. Clerk Dashboard webhooks call `/clerk-webhook` for `user.created`, `user.updated`, and `user.deleted`; the endpoint verifies signatures and syncs D1 `users`.
8. D1 role changes initiated by the app use `/user-role`, which mutates D1 and then updates Clerk metadata. Manual SQL changes require `/sync-clerk-metadata` because D1 has no outbound webhook trigger.
9. `/form-submit` uses `functions/_clerk-auth.js` to verify the Clerk token, check D1 roles, sync Clerk metadata from current D1 roles, and attach D1 user ownership to profiles/listings/claims/audit rows.
