...
| `js/auth-clerk.js` | Custom ClerkJS client integration. Replaces the legacy `/login` WPForms block with public login/register/verification panels, supports Google sign-in/sign-up, completes Clerk OAuth redirect callbacks before token checks, stores selected public registration role and post-login destination across OAuth redirects before `/auth-sync`, supports a login-only staff/admin variant for `/dashboard`, keeps inactive auth panels hidden, powers `/register`, exposes `window.FranchiseAuth`, supplies bearer tokens to protected form writes, has a public publishable-key fallback if `/auth-config` is stale or empty, sets Clerk's script-load publishable-key global/attributes, and loads locally copied ClerkJS before CDN fallbacks. | `/auth-config`, `/auth-sync`, `/login`, `/register`, `/dashboard`, `/daftar`, `js/form-06-submit-validation.js`, `/clerk/clerk.browser.js` |
| `css/auth-clerk.css` | Custom auth UI styling built on existing Astra/global colors. Explicitly preserves `[hidden]` tab/panel behavior against legacy display rules and styles login/register switch prompts plus Google OAuth buttons. | `/login`, `/register`, `/dashboard`, `/daftar` |
...
| `src/pages/dashboard/index.astro` | Static protected dashboard shell at `/dashboard`. Shows a same-URL login-only Clerk form for unauthenticated staff/admin users, keeps operational panels locked until `/dashboard-data` authorizes D1 `staff` or elevated `admin`, and renders overview metrics, unclaimed WhatsApp outreach with manual sent logging, data-quality warnings, listing edit suggestion form, admin approve/reject controls for suggestions and claims, lead summary, system health, publish state, and staff edit policy. Its styles are self-contained plus existing Elementor/Auth CSS; it does not depend on the missing legacy Astra dynamic CSS file. | `functions/dashboard-data.js`, `js/auth-clerk.js`, `css/auth-clerk.css`, Clerk, D1 |
...
| `migrations/0005_email_role_grants.sql` | Adds `email_role_grants` for pre-authorizing roles by normalized email before a real Clerk user id exists. Applied remotely to `franchise_db` on 2026-06-24; `admin@alampintar.org` has an active pending `admin` grant for Google SSO bootstrap. |
...
5. Email/password and Google OAuth are both supported. The custom browser bootstrap handles Clerk OAuth callback parameters before token checks, then clears stale callback parameters or navigates to the saved post-login URL.
6. Public Google registration stores the selected `franchisee`/`franchisor` role across the OAuth redirect, then syncs it after the Clerk session is active.
7. After sign-up or sign-in, `/auth-sync` verifies the Clerk session, upserts D1 `users`, applies active `email_role_grants`, inserts self-assignable `franchisee`/`franchisor` roles when requested, and pushes the D1 role snapshot into Clerk metadata.
8. Clerk Dashboard webhooks call `/clerk-webhook` for `user.created`, `user.updated`, and `user.deleted`; the endpoint verifies signatures and syncs D1 `users`.
9. D1 role changes initiated by the app use `/user-role`, which mutates D1 and then updates Clerk metadata. Manual SQL changes require `/sync-clerk-metadata` because D1 has no outbound webhook trigger.
10. `/form-submit` uses `functions/_clerk-auth.js` to verify the Clerk token, check D1 roles, sync Clerk metadata from current D1 roles, and attach D1 user ownership to profiles/listings/claims/audit rows.
