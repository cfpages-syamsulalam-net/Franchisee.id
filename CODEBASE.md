...
| `functions/profile-upload.js` | Protected owner media upload API for `/profil` listing assets. Requires Clerk/D1 auth, verifies listing ownership, validates logo/cover image and proposal PDF file types/sizes, writes files to the `FRANCHISE_ASSETS` R2 binding, records `franchise_assets`, updates the matching public listing media URL field, writes audit events, and queues static rebuild requests. | `js/profile-page.js`, `functions/_clerk-auth.js`, `functions/_site-publish-queue.js`, R2 `FRANCHISE_ASSETS`, D1 `franchise_assets`, `franchises`, `site_rebuild_requests` |
...
| `migrations/0006_saved_opportunities.sql` | Adds `franchise_saved_opportunities` so logged-in franchisee saved listings persist across devices. |
| `wrangler.toml` | Active Wrangler config for Cloudflare Pages output `dist`, shared D1 binding `franchise_db` using database UUID `812cd8ac-edd0-45d9-981f-c9a15358317b`, and R2 binding `FRANCHISE_ASSETS` for owner-uploaded listing media. It intentionally omits `account_id` because Pages config validation rejects that key. Remote commands should run through `npx cfman wrangler --account franchise-network ...`. |
