...
| 7. Public directory rebuild | In progress | D1-backed static HTML bridge generates root `/peluang-usaha/` output and a D1 snapshot; Astro static routes now generate `/peluang-usaha`, flat detail pages, `/rekomendasi`, `/populer`, `/abjad`, `/kategori`, category archives, top-level legacy category aliases, CSS-only image placeholders, cleaned metadata, and `/category` to `/kategori` redirects into `dist/`, then the build copies legacy static assets/pages without overwriting Astro output. Next add real popularity/view/lead metrics and verify deployed route precedence. |
