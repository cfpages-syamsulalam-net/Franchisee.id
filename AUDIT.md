...
| `functions/_ocr-job-runner.js` | 1,218 after OCR action/claiming split; new modules: `_ocr-job-actions.js` 100, `_ocr-job-claiming.js` 104 | Runner remains above 900 lines because it still owns provider execution, image loading, cache writes, rate/quota handling, proposal knowledge persistence, and OCR search/read-state orchestration. Dashboard retry/no-text actions and pending-job claiming/release are now extracted. | Keep this as the OCR execution orchestrator for now. Next split, only if OCR grows again: provider quota/rate-limit helpers or OCR search/read-state queries into dedicated modules. The two new modules are small and not refactor targets. | Extracted; watch |
