...
| Existing deterministic extraction missed many canonical candidate fields after the larger OCR backfill. | Expanded the shared proposal extractor for area/staff/setup, investment/fee/working-capital, contract, BEP range, omzet/profit, royalty basis/period, target-market, and support signals; added a replayable D1 backfill script; sanitized known source-noise watermarks from stored OCR text; remote replay increased candidate-bearing rows from 56 to 164 of 479 and now returns `changed=0`. The extractor now treats franchise fee/biaya lisensi/biaya kemitraan as one license/partnership fee field and avoids extracting total investment from a single fee row unless total/nilai investasi is explicit. | Implemented |
