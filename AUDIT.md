...
| `scripts/build-d1-franchise-pages.ts` | 767 | Builder mixes D1 fetch, snapshot shaping, file writing, manifest/prune behavior, remote access fallback, and structured location override query logic. | Split D1 fetch/snapshot writer/manifest pruning into modules after the Astro route bridge is stable. | Pending |
| `scripts/import-csv-to-d1.ts` | 736 | Importer mixes CSV parsing, validation, row mapping, SQL generation, and remote apply. | Split parser, mappers, and SQL writer before the next large import source is added. | Pending |
...
| `functions/form-submit.js` | 660 | Form API still handles multiple submission types, role checks, D1 writes, duplicate checks, test-data actions, and publish queueing in one file. | Split by workflow after dashboard/auth refactors: franchisee submit, franchisor submit, claim submit, and dev/test actions. | Pending |
