...
| One edit suggestion per OCR page would flood Review after structured replay. | Added a per-franchise OCR enrichment queue that groups structured candidates, keeps source page/excerpt context, skips already-filled canonical fields, and creates one pending `ocr_enrichment_bundle` suggestion whose JSON payload contains the actual editable fields for normal admin approval. | Implemented |
