...
| `functions/_dashboard-queries.js` | 711 after document evidence backfill was extracted to `_dashboard-review-evidence.js` (102 lines) | Dashboard read model handles overview, outreach, quality, claims, premium, editable listings with structured locations, publications, leads, health, telemetry, and audit summaries. Document proof matching is now delegated to the focused evidence helper. | Split read domains if new dashboard features continue to grow the file: `_dashboard-query-premium.js`, `_dashboard-query-quality.js`, `_dashboard-query-publication.js`, and `_dashboard-query-listings.js`. | Watch |
