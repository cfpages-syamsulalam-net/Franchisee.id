...
| `js/dashboard-outreach.js` | 442 after compact Outreach/Pipeline rendering | Owns the sales board, Google Contacts save, WhatsApp logging, status moves, next-action copy, overdue filters, notes, and burned-reason controls. It remains below the long-file threshold but is now the natural home for sales-pipeline UI behavior. | Split only if this crosses 600 lines or starts owning non-Outreach panels; likely first split is a pure card renderer helper. | Watch |
