...
| `functions/_dashboard-queries.js` | 830 after adding the sales outreach Kanban status read model | Dashboard read model handles overview, outreach, quality, claims, premium, editable listings with structured locations, publications, leads, health, telemetry, and audit summaries. Document proof matching is delegated to the focused evidence helper, but the outreach queue now also joins current pipeline/subscription status. | Split read domains after D1 storage/migration cleanup: first extract outreach reads into `_dashboard-outreach-queries.js`, then continue with `_dashboard-query-premium.js`, `_dashboard-query-quality.js`, `_dashboard-query-publication.js`, and `_dashboard-query-listings.js` as those domains grow. | Watch |
