...
| 9. Dashboards | In progress | `/dashboard` and `/dashboard-data` now implement the Franchisee.id admin/staff MVP plus an admin-only OCR configuration tab: protected metrics, outreach, quality/review, Premium/publication operations, masked OCR credential management, system health, audit writes, and rebuild queue writes. `DASHBOARD.md` remains the progress tracker for OCR job execution and future network-wide operations. |
