...
| `functions/profile-data.js` | 582 after profile API action split | Profile API facade owns auth/error handling, GET response composition, and remaining franchisor/listing/lead write workflows. Zod schemas, shared response utilities, account/role actions, franchisee actions, recommendation scoring, listing patch construction, and Premium workflows are delegated to helper modules. | Next extraction: move franchisor profile, owner listing edit, and lead status workflows into `_profile-franchisor-actions.js`. | In progress |
...
| `js/dashboard-operations.js` | New dashboard operations module | Owns Outreach rendering/logging, pending Premium payment review controls, publication status controls, lead summary, and system health rendering. | Keep operational dashboard workflow growth outside `js/dashboard-admin.js`. | Extracted |
...
| `js/profile-franchisee.js` | New profile franchisee renderer | Owns Minat Usaha and Peluang Saya rendering, including opportunity cards and inquiry history. | Keep API action workflows in `js/profile-page.js`. | Extracted |
| `js/profile-franchisor.js` | New profile franchisor renderer | Owns Data Brand, Listing Brand, distribution chips, media upload controls, and claim history rendering. | Keep upload/save workflows in `js/profile-page.js`. | Extracted |
...
| `functions/_profile-account.js` | New profile account/role action module | Owns Clerk+D1 account identity updates and additive public franchisee/franchisor role assignment. | Keep D1 authorization in `/profile-data`; do not add admin/staff self-service roles here. | Extracted |
| `functions/_profile-franchisee-actions.js` | New profile franchisee action module | Owns franchisee profile edits, inquiry creation, saved opportunity save/remove actions, missing-profile CTA metadata, and product-event writes. | Keep read-model loaders in `functions/profile-data.js` until GET composition is split. | Extracted |
