...
| 4. Auth foundation | In progress | Custom Clerk email/password and Google login/register surfaces, OAuth callback finalization before session checks, `/auth-config`, `/auth-sync`, `/clerk-webhook`, `/user-role`, `/sync-clerk-metadata`, D1 user mapping, email-based pre-login role grants, D1-to-Clerk metadata snapshots, and D1 role checks are implemented. `admin@alampintar.org` is pre-authorized in remote D1 through `email_role_grants` for first Google SSO login. Next redeploy and verify Google OAuth on Cloudflare Pages. |
