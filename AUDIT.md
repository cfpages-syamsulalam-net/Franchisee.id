...
Complete claim fields cannot establish control of a brand. Existing submit code assigned owner and published applicant-supplied listing data immediately. Submission now keeps the existing listing unchanged; only an admin recording independently checked evidence can approve. D1 triggers guard concurrent claim and review writes; remote guard installed. `docs/product/USER_JOURNEYS.md` now includes dishonest claimant, fake listing, inquiry harvesting and rushed review cases for all relevant personas. Remaining risk: new brand submissions still publish without independent ownership verification; controlled signed-in production QA for claim rejection/approval and downstream rebuild has not been run on a disposable listing. Wrangler migration bookkeeping lists earlier manually applied `0034` as unapplied; `0035` was executed directly and is repeatable.
...
`/get-franchises` previously spread every stored `raw_payload` key into published rows, including potential private NIB/HAKI or applicant contact fields. Its explicit Sheets source bypassed D1 publication filtering. Public reads now select named listing columns only; Sheets requests are refused and D1 failures return a generic response. The static page builder stops selecting raw payload data, and its row schema strips it from the rebuilt checked-in snapshot. The GitHub repository is public: prior commits still contain raw applicant fields, including NIB, HAKI, PIC and contact keys. Follow suggestion 119 for historical exposure handling. The regression check preserves unclaimed claim search and rejects private fields. Historical commits and any caches are not retroactively removed; the next code gate after data-safety follow-up is independent review of new-brand submissions (suggestion 117).
