...
| `js/dashboard-outreach.js` | 529 after compact indicator cleanup | Owns sales board, Google Contacts save, WhatsApp logging, status moves, next-action copy, overdue filters, notes, and burned-reason controls. It now exceeds 500 lines; this icon-only correction reuses existing render helpers and adds no new responsibility. | If sales rendering grows, extract `renderOutreachCard`, badge/chip renderers, and their status/escape inputs into one focused renderer module while keeping event and mutation handlers here. | Watch |
