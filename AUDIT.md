...
- Live mobile test found category selection navigated to `/peluang-usaha/kategori/` because backslashes vanished inside generated JS regex. Fixed source template escaping, checked emitted script, verified category slug target locally; live route dan hasil gabungan terverifikasi setelah follow-up push.- Audit UI/UX: `docs/ux/DIRECTORY_FILTER_UI_UX_AUDIT_2026-09-24.md`. Form kategori/kota/modal bisa digabung dan ikon sekunder mengurangi kebisingan tautan; label lokasi menyatakan batas data. Kopi Coba memiliki nilai mentah `FnB` dan D1 diperbarui untuk satu baris; renderer dan impor kini memakai label kanonis. Produksi dan visual desktop/mobile terverifikasi pada deploy `bf51534`; kombinasi makanan/minuman + Sidoarjo + 50–100 juta memberi 2 / 130 hasil.
...
- State transitions now have a dedicated cross-system tracker at `docs/architecture/STATE_TRANSITION_AUDIT.md`. The first P0/P1 pass added Pages Function method checks, Google Contacts OAuth expired/denied cleanup, and Premium lifecycle/email queue transition checks. The follow-up pass closed the remaining audit rows with Google Contacts per-contact reconciliation, outreach current-stage timestamps, claim transition matrix, field dictionary, OCR provider health policy, OCR stale-running recovery, publish queue coalescing, autosave stale-claim guards, and an R2/D1 migration runbook.
...
- Product/SEO planning now has explicit guardrails: `docs/ux/UI_REFERENCE_MAP.md` defines visual references before styling, `docs/product/SCOPE_TRIAGE.md` defines P0/P1/P2 scope handling for compound requests, and `docs/seo/TOPICAL_AUTHORITY_AND_DIRECTORY_SEO_PLAN.md` captures the requested `/peluang-usaha` category URL, card, copy, and topical authority direction before code implementation.
...
| Done | Outreach backward-move timestamp policy | Pipeline analytics now distinguish historical milestone dates from current stage through `stage_changed_at` and current-stage UI copy. |
| Done | Static publish queue coalescing/reconciliation audit | Matching active rebuild requests now coalesce and `pending_count` is reconciled from queue rows. |
...
| Done | OCR stale-running and terminal batch semantics | Stale `running` OCR jobs return to `pending`, and overdue failed batches no longer set `completed_at`. |
...
| --- | --- | --- |
| P0 | Record UI reference, scope triage, and SEO plan so the request is not lost. | Done |
| P1 | Implement `/peluang-usaha` compact card/search/CTA/category URL changes. | Done |
| P1 | Generate richer category landing metadata and copy from category label/count/useful decision points. | Done |
| P2 | Add related category/tool links and create a category/article content tracker with intent/status. | Done |
| P2 | Research, write, review, and publish the tracked articles. | Planned editorial work |
...
| `src/pages/dashboard/index.astro` dashboard shell/OCR panel | The route had grown past 900 lines and mixed the shell, Review, Operations, Premium, and OCR markup in one file. | Split Review, Operations/Premium, and OCR tab markup into `src/components/dashboard/DashboardReviewPanel.astro`, `DashboardOperationsPanel.astro`, and `DashboardOcrPanel.astro`. The route is now a 222-line assembler that keeps metadata plus CSS/script order. | Medium | Done |
...
| File / area | Why it is now a refactor candidate | Proposed split | Priority | Status |
| --- | --- | --- | --- | --- |
| `functions/_ocr-job-runner.js` result/read-state section | The file is still 1,172 lines after prior OCR extractions and the R2 text storage update. Grouped enrichment logic lives in `_ocr-enrichment-review.js`, text-object IO lives in `_ocr-text-store.js`, but result reads/search, row masking, cache handling, and dashboard payload shaping still remain inside the execution runner. | Before adding another OCR dashboard result feature, extract read/search/mask/cache helpers into `_ocr-results-read-model.js`; keep provider execution and job state transitions in `_ocr-job-runner.js`. | Medium | Planned |
...
| `src/pages/dashboard/index.astro` dashboard shell | The route shell was 962 lines after adding the fourth OCR guide card, dedicated OCR Review subpanel, and full-width pending edit review table. The change improved layout without adding runtime logic, but the static shell became large. | Extracted `DashboardReviewPanel.astro`, `DashboardOperationsPanel.astro`, and `DashboardOcrPanel.astro`; the route now stays focused on the shell, tab list, metadata injection, and script/style loader. | Medium | Done |
...
| `src/pages/dashboard/index.astro` dashboard shell | The route shell remained large and included another Outreach header action container alongside existing OCR/Review/Premium markup. The change was small, but the file continued to be a slow review surface for dashboard layout changes. | Completed the planned split for the largest Review/Operations/OCR sections while preserving script/style load order and browser data hooks. Outreach and Data Quality remain inline because they are compact. | Medium | Done |

## Refactor Candidates - 2026-07-16
...
| 7 | `css/dashboard-outreach.css` | Extract compact Outreach worklist and Pipeline Kanban styles so dense sales rows can evolve without expanding general operations CSS. | Done |
| 8 | Validation | Keep `pnpm run build`, `pnpm run dashboard:ocr:check`, and targeted client syntax checks as the minimum guard after dashboard CSS/module edits. | Done |
...
| `src/lib/franchise-static.ts` | 732 after directory document/content extraction | Astro-side renderer remains a long route-facing facade for listing/detail composition and template orchestration. Directory types, metadata/schema/template cleanup/category editorial rendering, category content profiles, ranking, contact, text, and generated assets now live in focused modules. | Next safe split: move listing/category/capital/city card and control markup into `franchise-directory-renderers.ts`; keep snapshot loading and page composition in the facade. | Extracted; watch |
...
| `functions/_ocr-job-runner.js` | 1,172 after OCR action/claiming/quota splits and R2 text storage wiring; new text helper: `_ocr-text-store.js` 60 | Runner remains above 900 lines because it still owns provider execution, image loading, cache reads/writes, proposal knowledge persistence, and OCR search/read-state orchestration. Dashboard retry/no-text actions, pending-job claiming/release, quota policy, grouped enrichment review, and object storage helpers are now extracted. | Keep this as the OCR execution orchestrator for now. Next split, only if OCR grows again: OCR search/read-state/cache queries into `_ocr-results-read-model.js`. The enrichment and text-store modules are small and not refactor targets. | Extracted; watch |
| `functions/_dashboard-schemas.js` | 132 after OCR schema split and granular review approval schema update | Dashboard action union and shared editable-listing write helpers. OCR action schemas moved out so this file stays a dashboard-wide contract facade. | Keep cross-dashboard schemas here; put feature-specific schema clusters in matching modules when a domain adds several actions. | Extracted |
...
| `functions/_dashboard-queries.js` | 681 after extracting Outreach/Pipeline reads | Dashboard read model still handles overview, quality, claims, premium, editable listings with structured locations, publications, leads, health, telemetry, and audit summaries. Document proof matching and Outreach/Pipeline are now delegated to focused helpers. | Continue with `_dashboard-query-premium.js`, `_dashboard-query-quality.js`, `_dashboard-query-publication.js`, and `_dashboard-query-listings.js` only as those domains grow enough to justify another behavior-preserving split. R2 text migration and migration-ledger cleanup are complete. | Watch |
| `src/lib/franchise-static-assets.ts` | 30-line compatibility facade; directory styles 575, client 196, content styles 129 | Placeholder rendering and asset exports stay stable while directory injection, CSS, browser behavior, and category editorial styles live in separate modules. The base directory style literal remains long but has one bounded responsibility. | Split base directory styles into layout/control and card/action constants only when the next visual pass touches both areas; keep current generated output contract covered by `pnpm run directory:check`. | Extracted; style module watch |
...
| `functions/_dashboard-actions.js` | 629 after shared location-write extraction and granular review approval update | Dashboard writes cover outreach, guided edits, quality refresh, granular field-level suggestion review, claims, publication, Premium settings/email/payment, audit, and rebuild queueing. Location normalization/write statement creation now lives in `functions/_location-writes.js`. | Split only when action growth resumes; likely `_dashboard-action-premium.js` and `_dashboard-action-review.js`. | Watch |
...
| `js/dashboard-review.js` | 650 | Owns Data Quality rendering, claim review rendering/actions, guided listing edit rows, admin location editor, edit suggestion submission, document/manual suggestion routing, review value formatting, per-field approval selection, OCR evidence diff rendering, and edit/claim/location actions. | Keep future review/data-quality/location workflow changes here instead of growing `js/dashboard-admin.js`; split pending review table/diff/evidence/value formatting/selection out if this crosses 700 lines. | Extracted; watch |
| `js/dashboard-operations.js` | 258 after Outreach was extracted to `js/dashboard-outreach.js` | Owns pending Premium payment review controls, publication status controls, lead summary, and system health rendering, while delegating Outreach board behavior to the focused Outreach module. | Keep operational dashboard workflow growth outside `js/dashboard-admin.js`; keep future sales-pipeline behavior in `js/dashboard-outreach.js`. | Extracted |
| `js/dashboard-outreach.js` | 560 after dashboard UI audit | Owns sales cards, Google Contacts save, WhatsApp logging, status moves, count and next-action copy, overdue filters, notes, and Burned reason controls. The Burned state gate and stable indicator slots keep this logic focused, but the file remains over 500 lines. | When the next sales renderer change arrives, extract pure card/badge/chip rendering into a focused module; keep event and mutation handlers here. Avoid splitting this stateful correction without a separate behavior-preserving check. | Watch |
...
| `src/lib/franchise-category.ts` | `getCategoryRouteEntries`, `getCategorySummaries`, canonical normalization, `categorySlug`, and `canonicalCategoryHref`. Implemented. | `renderCategoryIndexPage` remains in `franchise-static.ts` until category card rendering needs its own module. | Category aliases consolidate into canonical `/peluang-usaha/kategori/[slug]` links; `jasa-layanan` is the only intentional aggregate hub. |
...
- Cosmetic/metadata cleanup on 2026-06-21 and 2026-06-22 removed fake detail author/date blocks and standardized the then-current category links; the 2026-07-17 directory pass superseded those query links with canonical `/peluang-usaha/kategori/[slug]` routes.
...
## 2026-09-24 claim abuse and business journey follow-up
Complete claim fields cannot establish control of a brand. Existing submit code assigned owner and published applicant-supplied listing data immediately. Submission now keeps the existing listing unchanged; only an admin recording independently checked evidence can approve. D1 triggers guard concurrent claim and review writes; remote guard installed. `docs/product/USER_JOURNEYS.md` now includes dishonest claimant, fake listing, inquiry harvesting and rushed review cases for all relevant personas. New brand submissions now wait in private admin review before ownership or publication; controlled signed-in production QA for both claim and new-brand decisions remains to be run on disposable listings. Wrangler migration bookkeeping lists earlier manually applied `0034` as unapplied; `0035` was executed directly and is repeatable.
A new-brand submission with an exact existing name is blocked with a claim-flow instruction; distinct or disguised brand names still need independent new-brand review.

## Existing-brand registration choice
A late, plain duplicate-brand 409 made applicants fill an entire form before learning their brand already existed. `/daftar` now checks the first brand field, distinguishes published unclaimed/pending/managed states, lets the applicant claim an available listing, or shows the existing page. Contact details in the notice require an admin-approved claim on a published listing; account/private payload data stay out of the public projection. Production unclaimed-brand JSON uses `franchises.id`, whereas the previous claim lookup accepted only `legacy_row_id`; the handler now accepts both and records the real legacy ID. Local focused checks pass; production visual/mobile and authenticated end-to-end claim still need controlled browser QA.

## Public franchise export boundary
`/get-franchises` previously spread every stored `raw_payload` key into published rows, including potential private NIB/HAKI or applicant contact fields. Its explicit Sheets source bypassed D1 publication filtering. Public reads now select named listing columns only; Sheets requests are refused and D1 failures return a generic response. The static page builder stops selecting raw payload data, and its row schema strips it from the rebuilt checked-in snapshot. The GitHub repository is public: a reachable-commit audit found raw_payload keys but no populated NIB/HAKI values; populated PIC, email and phone values duplicated top-level fields already in those snapshots. Rewriting history for raw_payload alone would leave those values present. Follow suggestion 119 for the evidence and remaining contact-data policy decision. The regression check preserves unclaimed claim search and rejects private fields. Historical commits and any caches are not retroactively removed; the new-brand submission review now guards publication and ownership (suggestion 117); controlled production QA remains.
## 2026-09-25 focused OCR-assisted review

The managed OCR provider returned HTTP 429 for all 80 selected backend files, so it contributed no coverage. Three lower-cost native reviewers used OCR preview/rules and reviewed 36 selected source files across auth, browser, OCR, and Premium flows. They found five issues: claim suggestion XSS (high), cross-origin OAuth `next`, Clerk sync reactivation of inactive D1 users, duplicate OCR job claims, and renewal/expiration interleaving (medium). The five code fixes and focused regressions are in this change. This was a selected-file review, not a whole-repository clearance; live signed-in and D1 concurrency behavior still need deployment QA.

`functions/_ocr-job-runner.js` remains over 500 lines and spans provider routing, persistence, and job state. This fix changes state guards without adding another responsibility; a future extraction could move terminal transition construction to a dedicated module when that behavior next changes. `js/auth-clerk-core.js` also remains long; OAuth destination validation now has one helper, and a larger extraction would add churn to this security fix.
