...
- OCR execution UX audit: the persisted QStash batch path is useful as a background scheduler, but it is too indirect as the primary admin action because expired scheduler triggers can look stuck. The dashboard should default to a visible continuous-run loop that processes small chunks through the same server-side quota/rate-limit guards, while batch rows become observable/retryable background history.
...
| `Jalankan 100` made the scheduler/batch abstraction the primary UX, even though admins expect the button to do OCR now. | Repoint the main run button to dashboard continuous run: enqueue missing jobs, process small chunks (`run_ocr_jobs` max 5) repeatedly, respect provider rate/quota pauses, and allow stopping after the active chunk. Keep scheduler batch rows as background/history/retry support. | Implemented |
