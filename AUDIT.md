...
| `functions/_dashboard-queries.js` | 969 after measurable sales workflow metrics/instructions and OCR preview-only reads | Dashboard read model handles overview, outreach, quality, claims, premium, editable listings with structured locations, publications, leads, health, telemetry, and audit summaries. Document proof matching is delegated to the focused evidence helper, but outreach now joins pipeline, claim, subscription, SLA, overdue, and conversion-metric state. | Split read domains next: first extract outreach reads into `_dashboard-outreach-queries.js`, then continue with `_dashboard-query-premium.js`, `_dashboard-query-quality.js`, `_dashboard-query-publication.js`, and `_dashboard-query-listings.js` as those domains grow. R2 text migration and migration-ledger cleanup are complete. | Watch |
...
| `js/dashboard-outreach.js` | 441 after measurable sales workflow filters/instructions | Owns the sales board, Google Contacts save, WhatsApp logging, status moves, next-action copy, overdue filters, notes, and burned-reason controls. It remains below the long-file threshold but is now the natural home for sales-pipeline UI behavior. | Split only if this crosses 600 lines or starts owning non-Outreach panels; likely first split is a pure card renderer helper. | Watch |
