...
| `js/dashboard-review.js` review renderer | The module is 568 lines after splitting OCR bundles from generic review, improving field diff rendering, and adding source-backed OCR evidence rows. It remains under the deeper split threshold, but now owns quality, generic review, OCR review, claim review, and location editor rendering. | If another review/location workflow lands, extract pending review table/diff/evidence rendering into `dashboard-review-table.js` so the main module can stay focused on actions and panel wiring. | Medium | Planned |
...
| `js/dashboard-review.js` | 568 | Owns Data Quality rendering, claim review rendering/actions, guided listing edit rows, admin location editor, edit suggestion submission, OCR evidence diff rendering, and edit/claim/location actions. | Keep future review/data-quality/location workflow changes here instead of growing `js/dashboard-admin.js`; split pending review table/diff/evidence rendering out if this crosses 700 lines. | Extracted; watch |
