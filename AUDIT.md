...
- `/kategori`, `/kategori/[slug]`, `/category/[slug]`, and top-level category slugs: D1-backed static category index/archive routes with legacy slug aliases.
...
| 7. Public directory rebuild | In progress | D1-backed static HTML bridge generates root `/peluang-usaha/` output and a D1 snapshot; Astro static routes now generate `/peluang-usaha`, flat detail pages, `/rekomendasi`, `/populer`, `/abjad`, `/kategori`, category archives, compatibility category routes, and CSS-only image placeholders into `dist/`, then the build copies legacy static assets/pages without overwriting Astro output. Next add real popularity/view/lead metrics and verify deployed route precedence. |
...
- Directory route expansion on 2026-06-19: Astro now generates `/rekomendasi`, `/populer`, `/abjad`, `/kategori`, `/kategori/[slug]`, `/category/[slug]`, and top-level category slug archives from the same D1 snapshot. Verification build generated 259 Astro pages.
