...
| `functions/_profile-franchisee-actions.js` | New profile franchisee action module | Owns franchisee profile edits, inquiry creation, saved opportunity save/remove actions, missing-profile CTA metadata, and product-event writes. | Loader callbacks now come from `functions/_profile-read-model.js`; keep action writes here. | Extracted |
| `functions/_profile-franchisor-actions.js` | New profile franchisor/listing/lead action module | Owns franchisor profile edits, owner listing edit throttling, rebuild queueing, owned publication distribution reads, franchisor lead inbox reads, and lead status updates. | Keep D1 authorization in `/profile-data`; use `_profile-listing-patch.js` for allowed listing field changes. | Extracted |
| `functions/_profile-read-model.js` | New profile read-model module | Owns `/profile-data` GET payload composition, user/profile/listing/claim/recommendation/saved/inquiry/lead/premium reads, and franchisee loader callbacks. | Keep response keys stable for `/profil`; add smoke tests around this contract before new profile features. | Extracted |
