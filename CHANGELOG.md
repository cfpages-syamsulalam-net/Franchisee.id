...
- `functions/_form-submit-franchisor.js`, `functions/_form-submit-utils.js`, `js/form-02-claim-workflow.js` and `js/form-06-submit-validation.js` keep unmatched brand applications private with a clear pending notice and safe duplicate-name response. `migrations/0037_new_brand_review.sql` adds the review record, early owner/publication guards and a unique pending-name index. `migrations/0038_rejected_brand_resubmission.sql` makes rejection final so a rejected applicant cannot reserve the brand name. Both were applied to active D1 and recorded as migrations 37 and 38.
- `functions/_dashboard-actions.js`, `functions/_dashboard-queries.js`, `functions/_dashboard-schemas.js`, `functions/dashboard-data.js`, `js/dashboard-admin.js`, `js/dashboard-review.js` and `src/components/dashboard/DashboardReviewPanel.astro` add admin-only evidence-based approval/rejection, applicant-data visibility restriction, a rebuild on approval and private archiving on rejection.
- `scripts/check-new-brand-review-sql.py`, `scripts/check-new-brand-review.ts`, `scripts/check-brand-match.ts`, `scripts/check-claim-workflow.ts` cover database decisions and existing-brand paths.
- Implementation commit `86c6f456` reached `origin/main` and Cloudflare Pages production. Live dashboard review markup and browser script responded, public brand matching succeeded for an unmatched test name, and unauthenticated `/dashboard-data` returned 401. `docs/architecture/NEW_BRAND_REVIEW.md`, `docs/README.md`, `docs/product/NEXT_STEPS.md`, `docs/product/USER_JOURNEYS.md`, `AUDIT.md`, `SUGGESTION.md`, `CODEBASE.md` and `.context/session-20260924-2325.md` document the gate, next testing step and corrected historical raw-payload findings.
...
- `functions/_form-submit-utils.js` and `functions/_form-submit-franchisor.js` require an exact ownerless unclaimed listing and store a pending claim plus private profile without changing the listing, owner or public rebuild queue. New-brand submission rejects exact names of existing listings so it cannot bypass the claim flow.
...
- `scripts/copy-legacy-static.mjs`: Changed the injected legacy legal footer row to a high-contrast black/yellow Franchisee.id strip without a top gap, so Privacy Policy and Terms of Service links remain readable and visually integrated on copied legacy footers.
