...
## Existing-brand first choice on the franchisor form
The `brand_name` field checks an exact, trimmed, case-insensitive name through public-safe `GET /brand-match?name=...`. Matching published unclaimed listings offer **Klaim listing** only when no claim is pending; a managed listing offers its existing public page. PIC/phone appear in the notice only for a published listing with an admin-approved ownership claim. Multiple exact-name listings are shown separately. The new-listing submit guard still returns `BRAND_ALREADY_LISTED` with these same safe matches if the early check was missed. `claim_id` deep links select the exact unclaimed listing, including duplicate names. Claim lookup accepts either the current listing ID used by `json/unclaimed-brands.json` or its legacy row ID; the claim record stores the actual legacy row ID. No form fields were removed.
